...
export const queueController = {
  ...
  dashboard: asyncHandler(async (req, res) => res.json({ success: true, data: await queueService.getPatientDashboard(req.user.id) })),
  patients: asyncHandler(async (req, res) => res.json({ success: true, data: await queueService.getPatients(req.user.id) })),

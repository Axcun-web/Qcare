...
export const queueService = {
  async listDoctors() { return prisma.doctor.findMany({ where: { isActive: true }, include: { clinic: { select: { id: true, nama: true, alamat: true } }, jadwalPraktik: { where: { kuotaAntrean: { gt: 0 } }, select: { id: true, hari: true, jamMulai: true, jamSelesai: true, kuotaAntrean: true } } }, orderBy: { nama: "asc" } }); },
  async create(userId, input) {
    ...
    let patient;
    const hubungan = input.registerFor === "self" ? "Diri sendiri" : "Orang Lain";
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (existing) throw ApiError.conflict("Pasien sudah memiliki antrean aktif. Silakan selesaikan atau batalkan terlebih dahulu.");
      const countForNumber = await tx.antrean.count({ where: { clinicId: doctor.clinicId, tanggal: date } });
      const countForQuota = await tx.antrean.count({ where: { jadwalId, tanggal: date, status: { notIn: ["DIBATALKAN"] } } });
      if (countForQuota >= schedule.kuotaAntrean) throw ApiError.conflict("Kuota antrean sudah penuh");
      const entry = await tx.antrean.create({ data: { recordPasienId: patient.id, doctorId, clinicId: doctor.clinicId, jadwalId, nomorAntrean: countForNumber + 1, tanggal: date, sumber: "ONLINE" }, include: queueInclude });
      await tx.predictionHistory.create({ data: { antreanId: entry.id, doctorId, tanggal: date, estimasiDurasiAntrean: 10, estimasiMenit: countForQuota * 10, sumberEstimasi: "DEFAULT" } });
      return entry;
    });
  ...
  async getPatients(userId) { return prisma.recordPasien.findMany({ where: { userId: BigInt(userId) }, orderBy: { createdAt: "asc" } }); },

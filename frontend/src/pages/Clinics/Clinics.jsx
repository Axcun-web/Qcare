...
// Sub-komponen per kartu klinik agar memiliki state filter masing-masing
function ClinicCard({ clinic }) {
  const [selectedSpecialty, setSelectedSpecialty] = useState("Semua");

  // Mengambil daftar spesialisasi unik dari dokter yang ada di klinik ini
  const availableSpecialties = [
    "Semua",
    ...Array.from(
      new Set(
        (clinic.doctors || [])
          .map((doc) => doc.spesialisasi)
          .filter(Boolean)
      )
    ),
  ];

  // Filter dokter berdasarkan spesialisasi yang dipilih
  const filteredDoctors = (clinic.doctors || []).filter((doc) => {
    if (selectedSpecialty === "Semua") return true;
    return doc.spesialisasi === selectedSpecialty;
  });
    ...
    <article className="clinic-card">
      ...
      <div className="doctor-section">
        <div className="doctor-section-header">
          ...
          {/* Opsi Filter Kategori Dokter */}
          {clinic.doctors && clinic.doctors.length > 0 && availableSpecialties.length > 1 && (
            <div className="specialty-filter-container">
              <span className="filter-label">Filter Dokter:</span>
        ...
        {clinic.doctors && clinic.doctors.length > 0 ? (
          filteredDoctors.length > 0 ? (
                    ...
                    {doc.jadwalPraktik && doc.jadwalPraktik.length > 0 ? (
                      doc.jadwalPraktik.map((j) => (
                        <div key={j.id} className="schedule-badge">
                          {j.hari.charAt(0).toUpperCase() + j.hari.slice(1).toLowerCase()}:{" "}
                          {new Date(j.jamMulai).toLocaleTimeString("id-ID", {
                            timeZone: "UTC",
                            hour12: false,
                            hour: "2-digit",
                            minute: "2-digit",
                          ...
                          -{" "}
                          {new Date(j.jamSelesai).toLocaleTimeString("id-ID", {
                            timeZone: "UTC",
                            hour12: false,
                            hour: "2-digit",
                            minute: "2-digit",
            ...
            <p className="empty-doctors">Tidak ada dokter untuk spesialisasi "{selectedSpecialty}".</p>
          )
        ) : (
